...
object COPJob2 {
  def main(args: Array[String]) = {
    ...
    var sparkBuilder = SparkSession
      .builder()
      //.master("local[*]")
      .appName(s"DataQuality-Only-COP-Filtered-Experimental")
    ...
    //val ds = spark.read.json("/home/alvaro/datasets/hidrocantabrico_split.json")
      val ds = spark.read.schema(Common.deserializeSchema(Common.schemaStr)).json("hdfs://10.141.10.111:8020/user/snape/cbd/hidrocantabrico_dq_filtered.json")
      //val hidrocantabrico = spark.read.json("/home/alvaro/datasets/endesa_datos_agregados_split.json")
      //  .map()
      .withColumn("output", explode(array(executeCOPUdf(struct(dqin: _*))))) // TODO Bastaría con esta column
