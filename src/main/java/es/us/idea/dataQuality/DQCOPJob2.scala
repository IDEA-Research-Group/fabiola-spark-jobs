...
object DQCOPJob2 {
  def main(args: Array[String]) = {
    ...
    var sparkBuilder = SparkSession
      .builder()
      //.master("local[*]")
      .appName(s"DataQuality-DQCOPJob2-Experimental")
    ...
    val executeCOPandDQUdf = udf((row: Row) => {
      val dqOout = dqFunction(row)
      if(dqOout.dataQualityQualitative.isDefined)
        if(dqOout.dataQualityQualitative.get == "good"){
          val copOut = ClassCompiler.callMethod(classStr, row)
          DQCOPOutout(Option(copOut), Option(dqOout))
        } else
          DQCOPOutout(None, Option(dqOout))
      else
        DQCOPOutout(None, Option(dqOout))
    })
    ...
    val ds = spark.read.schema(Common.deserializeSchema(Common.schemaStr)).json("hdfs://10.141.10.111:8020/user/snape/cbd/hidrocantabrico.json")
      ...
      //  .map()
      .withColumn("output", explode(array(executeCOPandDQUdf(struct(dqin: _*))))) // TODO Bastaría con esta column
